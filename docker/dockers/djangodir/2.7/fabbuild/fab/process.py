...
def stopmiddleware(middlewarename,prodcmddir,stopcmd):
    # todo 如果中间名写错的处理
    ...
    if middlewarename.upper().find('TOMCAT') >= 0:
        ...
        stopjboss(prodcmddir,suffix,stopcmd)

def startmiddleware(middlewarename,prodcmddir,startcmd):
    # todo 如果中间件名写错的处理
    osplatform = getosplatform()
    if osplatform == "Windows":
        suffix = "bat"
    ...
    if middlewarename.upper().find('TOMCAT') >= 0:
        ...
        startjboss(prodcmddir,suffix,startcmd)
...
def startjboss(prodcmddir,suffix,startcmd):
    new_env = os.environ.copy()
    new_env['JAVA_HOME'] = config.configs.javahome
    new_env['JBOSS_HOME'] = prodcmddir
    if suffix == "bat":
        cmd = prodcmddir + os.path.sep + "bin" + os.path.sep + "standalone." +suffix
    else:
        cmd = startcmd
    p = subprocess.Popen(cmd,stdout=subprocess.PIPE, stderr=subprocess.STDOUT,env=new_env,shell = True)
    # while p.poll() is None:
    #     line = p.stdout.readline()
    #     line = line.strip()
    #     if line:
...
def stopjboss(prodcmddir,suffix,stopcmd):
    new_env = os.environ.copy()
    new_env['JAVA_HOME'] = config.configs.javahome
    new_env['JBOSS_HOME'] = prodcmddir
    if suffix == "bat":
...
def getProd(middlewarename):
    ...
    for mw in middleware:
        if mw[0] == middlewarename:
            ...
            stopcmd = mw[1].get('stopcmd')
            startcmd = mw[1].get('startcmd')
            break

    return prodPlace,prodcmddir,stopcmd,startcmd